# # app.py
# # ============================================
# # Dashboard de Acidentes — Goiás / Goiânia
# # Feito para os dados consolidados em: data/processed_data/base_acidente.csv
# # ============================================
...
#     df["TAXA_MORTALIDADE"] = (
#         df["mortos"] / df["pessoas"].replace(0, np.nan)
#     ).fillna(0)
...
#     df_rmg = df_go[df_go["municipio"].isin(RMG_MUNICIPIOS)].copy()
#     return df_go, df_rmg

# def kpis_bloco(df, titulo):
#     col1, col2, col3, col4 = st.columns(4)
#     with col1: st.metric("Registros", f"{len(df):,}".replace(",",".")) 
#     with col2: st.metric("Mortos", int(df["mortos"].sum()))
#     with col3: st.metric("Feridos (L+G)", int(df["TOTAL_FERIDOS"].sum()))
#     taxa = (df["mortos"].sum()/len(df)*100) if len(df)>0 else 0
...
#     g=df.groupby("br").agg(oc=("br","size"),mt=("mortos","sum")).reset_index()
#     if len(g)==0: return g.assign(indice=0.0,taxa=0.0)
#     g["taxa"]=g["mt"]/g["oc"].replace(0,np.nan)
#     g["indice"]=(0.4*(g["oc"]/g["oc"].max())+0.6*(g["mt"]/max(g["mt"].max(),1)))*10
...
# # ======== MODELAGEM ============
# # -------------------------------
# def make_splits(df_go):
#     y=(df_go["mortos"]>0).astype(int)
#     num=["km","veiculos","pessoas","TOTAL_FERIDOS","HORA"]
#     num=[c for c in num if c in df_go.columns]
#     cat=["tipo_acidente","causa_acidente","tipo_pista","tracado_via","uso_solo",
#          "PERIODO_DIA","dia_semana","municipio","br"]
#     cat=[c for c in cat if c in df_go.columns]
#     X=df_go[num+cat].copy()
...
# def train_models(X_train,y_train,ct):
...
#     return {"LogisticRegression":pipe_lr,"HistGradientBoosting":pipe_hgb}
...
#     with c2: st.metric("PR-AUC",f"{pr:.3f}")
#     with c3:
...
# with tab1:
#     kpis_bloco(df_go,"Resumo geral GO")
# with tab2:
#     ag_go=df_go.groupby("year").agg(oc=("year","size"),mt=("mortos","sum")).reset_index()
#     ag_rmg=df_rmg.groupby("year").agg(oc=("year","size"),mt=("mortos","sum")).reset_index()
...
#     X_train,X_test,y_train,y_test,ct,meta=make_splits(df_go)
#     models=train_models(X_train,y_train,ct)
#     for name,pipe in models.items():
#         evaluate_model(name,pipe,X_test,y_test)

# st.caption("Feito com ❤️ — Willgnner Ferreira Santos")
